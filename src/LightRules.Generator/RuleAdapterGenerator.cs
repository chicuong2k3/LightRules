            ...
            string BuildParameterBindings(IMethodSymbol method, string factsVar = "facts")
            {
                ...
                for (int i = 0; i < method.Parameters.Length; i++)
                {
                    ...
                    if (hasFact)
                    {
                        ...
                        lines.Add($"if (!{factsVar}.TryGetValue<{paramType}>(\"{factNameLiteral}\", out var {localVar})) throw new global::LightRules.Core.NoSuchFactException(\"No fact named '{factNameLiteral}' found\", \"{factNameLiteral}\");");
                    }
                    else
                    {

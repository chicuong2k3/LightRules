...
namespace LightRules.Generator
{
    ...
    public class RuleAdapterGenerator : ISourceGenerator
    {
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            foreach (var syntaxTree in compilation.SyntaxTrees)
            {
                ...
                foreach (var cls in classDecls)
                {
                    ...
                    var nameExpr = nameLiteral != null ? "\"" + nameLiteral.Replace("\\", "\\\\").Replace("\"", "\\\"") + "\"" : "\"" + symbol.Name + "\"";
                    var descExpr = descriptionLiteral != null ? "\"" + descriptionLiteral.Replace("\\", "\\\\").Replace("\"", "\\\"") + "\"" : "\"" + symbol.Name + "\"";
                    var prioExpr = priorityLiteral != int.MinValue ? priorityLiteral.ToString() : "IRule.DefaultPriority";
                    // Generate factory lambda that creates POCO and passes to adapter - no reflection required
                    var pocoFullName = symbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
                    var factoryExpr = $"() => new global::{adapterFullName}(new {pocoFullName}())";
                    registryEntries.Add($"new LightRules.Discovery.RuleMetadata(typeof(global::{adapterFullName}), {nameExpr}, {descExpr}, {prioExpr}, true, new string[0], {factoryExpr})");
                }
            }
        ...
        private string GenerateAdapterFor(INamedTypeSymbol symbol, INamedTypeSymbol? conditionAttr, INamedTypeSymbol? actionAttr, INamedTypeSymbol? factAttr, INamedTypeSymbol? priorityAttr, GeneratorExecutionContext context)
        {
            ...
            // Use compile-time type name instead of runtime GetType().Name to avoid reflection
            string fallbackName = "\"" + simpleTypeName + "\"";
            string nameExpression = nameLiteral != null ? "\"" + nameLiteral.Replace("\\", "\\\\").Replace("\"", "\\\"") + "\"" : fallbackName;
            string descriptionExpression = descriptionLiteral != null ? "\"" + descriptionLiteral.Replace("\\", "\\\\").Replace("\"", "\\\"") + "\"" : fallbackName;
            ...
            string BuildParameterBindings(IMethodSymbol method, string factsVar = "facts")
            {
                ...
                for (int i = 0; i < method.Parameters.Length; i++)
                {
                    ...
                    if (hasFact)
                    {
                        ...
                        lines.Add($"if (!{factsVar}.TryGetFactValue<{paramType}>(\"{factNameLiteral}\", out var {localVar})) throw new global::LightRules.Core.NoSuchFactException(\"No fact named '{factNameLiteral}' found\", \"{factNameLiteral}\");");
                    }
                    else
                    {
            ...
            string BuildArgumentList(IMethodSymbol method, string factsVar = "facts")
            {
            ...
            {
                var preAndArgs = BuildParameterBindings(act, "current");
                var argList = BuildArgumentList(act, "current");
                var returnsFact = !act.ReturnsVoid;
                if (returnsFact)
                {
                    source += $"            {preAndArgs}\n            current = _target.{act.Name}({argList});\n";
                }
                else
                {
        ...
        private string GenerateRegistrySource(List<string> entries)
        {
            ...
            // Generate a ModuleInitializer that auto-registers rules - no reflection required
            var source = $@"// <auto-generated/>
using System;
...
{{
    /// <summary>
    /// Auto-generated module initializer that registers rules into GlobalRuleRegistry.
    /// This runs automatically when the assembly loads - no reflection required.
    /// </summary>
    internal static class RuleRegistryInitializer
    {{
        [ModuleInitializer]
        internal static void Initialize()
        {{
            GlobalRuleRegistry.Register(
                {body}
            );
        }}
    }}
    ...
    /// <summary>
    /// Static registry of rules in this assembly. Access via GlobalRuleRegistry or RuleDiscovery.Discover().
    /// </summary>

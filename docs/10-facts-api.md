...
A compact reference table for the `Facts` collection API. **Note: `Facts` is an immutable record and thread-safe** - all
mutation methods return a new `Facts` instance.
...
| `TryGetFactValue<T>(string name, out T? value)` | Safe typed read (recommended)                 | Returns `true` only when a fact exists and its runtime value is assignable to `T`. Sets `value` accordingly. |
